...
import { designCards, type DesignCard } from "../../../lib/design-cards";
        ...
        {designCards.map((card: DesignCard) => (
          <Link key={card.id} href={`/styles/${card.id}`} className="group block rounded-2xl border border-light bg-surface p-7 shadow-sm hover:shadow-lg hover:border-dark transition-all duration-150">
            ...
            <div className="flex gap-2 flex-wrap mb-3">
              {card.tags.map((t: DesignCard["tags"][number]) => (
            ...
            <div className="flex gap-1 mt-2">
              {card.palette.slice(0, 4).map((p: DesignCard["palette"][number], i: number) => (
