...
import { Search, Globe, AlertCircle, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react";
...
function CardThumbnail({ run }: { run: RunCard }) {
  ...
  if (!immediatePreview && lazyPreview === null) {
    return <div ref={containerRef} className="w-full h-full bg-surface-soft animate-pulse" />;
  }
  ...
  return (
    <section className="py-12 md:py-16 bg-page">
      <div className="container-custom max-w-6xl mx-auto px-6">
        ...
        {!loading && !error && totalPages > 1 && (
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link
                href="/styles"
                className="text-sm font-semibold text-cta hover:opacity-80 transition-opacity whitespace-nowrap"
              >
                View all →
              </Link>
            </div>
        ...
        {/* Count + see all link fallback if only 1 page */}
